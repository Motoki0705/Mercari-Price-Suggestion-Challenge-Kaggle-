...
class AddPositionalEncoding(tf.keras.layers.Layer):
    def call(self, inputs: tf.Tensor) -> tf.Tensor:
        fl_type = inputs.dtype #dtype=float32
        shape = tf.shape(inputs) #[batch_size, length, depth]
        
        length = tf.cast(shape[1], fl_type)
        depth = tf.cast(shape[2], fl_type)
        ...
        return inputs + pos_matrix #[length, depth]
