...
class FFN(tf.keras.models.Model):
    ...
    def build(self, input_shape):
        # レイヤーに対して入力の形状に基づいて初期化を行う
        self.dense_4_layer.build(input_shape)
        output_shape = (input_shape[0], input_shape[1], self.embedding_dim * 4)
        self.dense_1_layer.build(output_shape)
        self.dropout_layer.build(output_shape)
        super().build(input_shape)  # 親クラスの build() を呼び出す
